...
class OpenRaveManager(object):
    ...
    def check_segment_validity(self, start_configuration, action):
        ...
        for step in steps:
            step = step[1:2] # since valid configuration function takes only 4 joint values
    ...
    def is_near_goal(self, current_configuration):
        ...
        if distance < self.config['path']['goal_distance']:
    ...
    def step(self, action):
        ...
        action = np.pad(action, (1, 3), 'constant', constant_values = (0,))  # action to be taken is for 4 joints only since first joint is always disabled
        reward = 0.0
        distance, action = self.truncate_actions(current_robot_configuration, action)
        reward -= self.config['reward']['truncate_penalty']

        if self.check_segment_validity(current_robot_configuration, action):
            is_near_goal, distance = self.is_near_goal(self.robot.GetDOFValues())
            if is_near_goal:
                reward += 2.0
                print(distance)
                print('Pass')
    ...
    def add_markers(self):
        self.robot.SetDOFValues(self.robot_start_configuration)
        transform = self.robot.GetLinks()[-1].GetTransform()
        body = openravepy.RaveCreateKinBody(self.env, '')
        body.SetName('start')
        body.InitFromSpheres(np.array([[0, 0, 0, 0.03]]),
                            True)
        self.env.Add(body, True)
        body.SetTransform(transform)
        body.GetLinks()[0].GetGeometries()[0].SetDiffuseColor(np.array([0, 0, 1, 1]))
        self.markers.append(body)
        self.robot.SetDOFValues(self.robot_goal_configuration)
        transform = self.robot.GetLinks()[-1].GetTransform()
        body = openravepy.RaveCreateKinBody(self.env, '')
        body.SetName('goal')
        body.InitFromSpheres(np.array([[0, 0, 0, 0.03]]),
        ...
        self.env.Add(body, True)
        body.SetTransform(transform)
        body.GetLinks()[0].GetGeometries()[0].SetDiffuseColor(np.array([0, 1, 0, 1]))
        self.markers.append(body)
        self.robot.SetDOFValues(self.robot_start_configuration)

    def delete_markers(self):
        for marker in self.markers:
            self.env.Remove(marker)
        self.markers = []
    ...
    def is_valid_configuration(self, joints):
        self.robot.SetDOFValues(joints, [1])
    ...
    def initialize_robot_position(self):
        self.robot.SetActiveDOFs(range(1, 2))
        ...
        while not valid_configuration:
            attempts += 1
            joint_values = []
            for i in range(1, self.robot.GetDOF() - 3):
